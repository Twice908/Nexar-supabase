...
//       ? `<div class="driver-chip-rating">★ ${avg.avg} · ${avg.count} rating${avg.count === 1 ? '' : 's'}</div>`
//       : '';

//     // Contact buttons visible only until 6h after ride end.
//     const SIX_HOURS = 6 * 60 * 60 * 1000;
//     const endedAt = ride.completedAt ? new Date(ride.completedAt).getTime()
//                    : (ride.cancelledAt ? new Date(ride.cancelledAt).getTime() : null);
//     const contactStillVisible = !endedAt || (Date.now() - endedAt) < SIX_HOURS;

//     const contactBtns = (driverUser.mobile && contactStillVisible) ? `
//       <div class="passenger-contact-buttons">
//         ${buildTelUrl(driverUser.mobile) ? `<a href="${buildTelUrl(driverUser.mobile)}" class="btn btn-secondary btn-sm passenger-contact-btn" title="Call Nexar">${iconPhone()}<span>Call</span></a>` : ''}
//         <a href="${buildWhatsAppUrl(driverUser.mobile, `Hi ${driverUser.name}, this is about our Nexar ride on ${ride.date}.`)}" target="_blank" rel="noopener" class="btn btn-secondary btn-sm passenger-contact-btn" title="WhatsApp Nexar">${iconWhatsApp()}<span>WhatsApp</span></a>
//       </div>
//     ` : '';

//     // Rating block for the driver, shown on completed rides.
//     const existingRating = window.app.store.getRatingFor(ride.id, user.email, driverUser.email);
//     const reviewBlock = (ride.status === 'completed') ? (
//       existingRating ? `
//         <div class="passenger-review-note">
//           You rated ${'★'.repeat(existingRating.stars)}${'☆'.repeat(5 - existingRating.stars)}
//           ${existingRating.comment ? `<div class="passenger-review-comment">"${escapeHtml(existingRating.comment)}"</div>` : ''}
//         </div>
//       ` : `
//         <div class="passenger-review-block" data-rate-row="${ride.id}|${driverUser.email}">
//           <div class="star-picker" data-star-target="${ride.id}|${driverUser.email}">
//             ${[1,2,3,4,5].map(n => `
//               <button type="button" class="star-btn" data-stars="${n}" onclick="app.pickStars('${ride.id}', '${driverUser.email}', ${n}, this)">★</button>
//             `).join('')}
//           </div>
//           <div class="rate-comment-row" style="margin-top:6px; display:none;" data-comment-for="${ride.id}|${driverUser.email}">
//             <input type="text" class="field-input" style="height:36px; font-size:13px;" placeholder="Comment (optional)" data-comment-input="${ride.id}|${driverUser.email}">
//             <button class="btn btn-primary btn-sm" style="margin-top:6px; width:100%;" onclick="app.submitRating('${ride.id}', '${driverUser.email}', this)">Submit rating</button>
//           </div>
//         </div>
//       `
//     ) : '';

//     const hasMenuContent = contactBtns || reviewBlock;
...
//         </div>
//         ${hasMenuContent ? `
//           <details class="passenger-actions-menu" name="passenger-actions">
//             <summary class="passenger-actions-trigger">Actions</summary>
//             <div class="passenger-actions-panel">
//               ${contactBtns}
//               ${reviewBlock}
//             </div>
//           </details>
...
//       <div class="ride-subsection-head"><span class="meta">Passengers</span><span class="ride-subsection-count">${ride.passengers.length}</span></div>
...
//         // ---------- Action buttons: only while ride is active ----------
//         const showActions = ride.status === 'active';
//         // Onboard: enabled only when passenger hasn't been picked up yet
//         const onboardDisabled = picked ? 'disabled' : '';
//         // Dropped: enabled only AFTER onboarding and BEFORE dropping
...
//             <button
//               class="btn btn-sm passenger-action-btn ${picked ? '' : 'passenger-action-btn-primary'}"
//               data-pickup-btn="${ride.id}|${email}"
//               ${onboardDisabled}
//               onclick="app.markPickedUp('${ride.id}', '${email}', this)">${picked ? 'Onboarded ✓' : 'Onboard'}</button>
//             <button
//               class="btn btn-sm passenger-action-btn ${droppedIsPrimary}"
//               data-drop-btn="${ride.id}|${email}"
//               ${droppedDisabled}
//               onclick="app.markDropped('${ride.id}', '${email}', this)">${dropped ? 'Dropped ✓' : 'Dropped'}</button>
//           </div>
//         ` : '';

//         // ---------- Contact buttons: visible until 6h after ride end ----------
//         const SIX_HOURS = 6 * 60 * 60 * 1000;
//         const endedAt = ride.completedAt ? new Date(ride.completedAt).getTime()
//                        : (ride.cancelledAt ? new Date(ride.cancelledAt).getTime() : null);
//         const contactStillVisible = !endedAt || (Date.now() - endedAt) < SIX_HOURS;
...
//         const contactBtns = (pPhone && contactStillVisible) ? `
//           <a href="${buildTelUrl(pPhone)}" class="btn btn-secondary btn-sm passenger-contact-btn" title="Call passenger" aria-label="Call passenger">${iconPhone()}<span>Call</span></a>
//           <a href="${buildWhatsAppUrl(pPhone, `Hi ${name}, about our Nexar ride on ${ride.date}.`)}" target="_blank" rel="noopener" class="btn btn-secondary btn-sm passenger-contact-btn" title="WhatsApp passenger" aria-label="WhatsApp passenger">${iconWhatsApp()}<span>WhatsApp</span></a>
//         ` : '';

//         // ---------- Review block: only on completed rides ----------
//         const existingRating = window.app.store.getRatingFor(ride.id, user.email, email);
//         const reviewBlock = (ride.status === 'completed') ? (
//           existingRating ? `
//             <div class="passenger-review-note">
//               You rated ${'★'.repeat(existingRating.stars)}${'☆'.repeat(5 - existingRating.stars)}
...
//             </div>
//           ` : `
//             <div class="passenger-review-block" data-rate-row="${ride.id}|${email}">
...
//                 <input type="text" class="field-input" style="height:36px; font-size:13px;" placeholder="Comment (optional)" data-comment-input="${ride.id}|${email}">
//                 <button class="btn btn-primary btn-sm" style="margin-top:6px; width:100%;" onclick="app.submitRating('${ride.id}', '${email}', this)">Submit rating</button>
//               </div>
//             </div>
//           `
//         ) : '';
...
//                 <div class="passenger-row-heading">
//                   <div class="passenger-name">${escapeHtml(name)}</div>
//                   ${statusPill}
//                   ${(contactBtns || actionsRow || reviewBlock) ? `
//                     <details class="passenger-actions-menu" name="passenger-actions">
//                       <summary class="passenger-actions-trigger">Actions</summary>
//                       <div class="passenger-actions-panel">
//                         ${contactBtns ? `<div class="passenger-contact-buttons">${contactBtns}</div>` : ''}
//                         ${actionsRow}
//                         ${reviewBlock}
//                       </div>
//                     </details>
...
//     <div class="passenger-list">
//       <div class="ride-subsection-head"><span class="meta">Riders</span><span class="ride-subsection-count">${ride.passengers.length}</span></div>
//       ${(ride.pickupOrder && ride.pickupOrder.length > 0 ? ride.pickupOrder : ride.passengers).map((email, i) => {
//         const g = (ride.group || []).find(x => x.email === email);
//         const p = window.app.store.getUserByEmail(email);
//         const name = p?.name || g?.name || email;
...
//   return `
//     <div class="ride-card ${past ? 'is-past' : ''} ride-card-${ride.status.replace(/\s+/g, '-')}">
//       <div class="ride-card-head">
//         <div>
//           <div class="ride-card-title">${ride.tripType === 'morning' ? 'Morning' : 'Evening'} ride</div>
...
//           <div class="timeline ride-live-status">
...
//   return `
//     <div class="ride-utility-actions">
//       ${mapsUrl ? `<a href="${mapsUrl}" target="_blank" rel="noopener" class="btn btn-secondary btn-sm ride-utility-btn">
//         ${iconMap()} <span>Open in Maps</span>
//       </a>` : ''}
//       <a href="${shareUrl}" target="_blank" rel="noopener" class="btn btn-outline btn-sm ride-utility-btn">
...
//     }
//   }
//   return btns.length ? `<div class="ride-actions ride-driver-actions">${btns.join('')}</div>` : '';
// }
...
export function RidesView({ user, rides }) {
  ...
  const upcoming = mine.filter(r => r.date >= todayStr)
    .sort((a, b) => {
      const p = ridePriority(a.status) - ridePriority(b.status);
      if (p !== 0) return p;
...
// ——— Ride card state resolver ———
function rideVisualState(ride, past) {
  const now = Date.now();
  const endedAt = ride.completedAt ? new Date(ride.completedAt).getTime()
                 : (ride.cancelledAt ? new Date(ride.cancelledAt).getTime() : null);
  const THREE_HOURS = 3 * 60 * 60 * 1000;
  const recentlyEnded = endedAt && (now - endedAt) < THREE_HOURS;

  const isActive = ride.status === 'active' || ride.status === 'started';
  const isMatched = ride.status === 'matched' || ride.status === 'matching';
  const isDead = ride.status === 'cancelled' || ride.status === 'no match' || past === true;

  let cardClass = '';
  if (isActive)            cardClass = 'ride-card-active';
  else if (recentlyEnded)  cardClass = 'ride-card-recent';
  else if (isDead || (endedAt && !recentlyEnded)) cardClass = 'is-dormant';

  // Menu is only visible when the ride is actually in progress, or recently
  // completed (within the 3-hour rating window). Before the ride starts
  // (matched / matching) there is nothing to call or rate yet.
  const menuVisible =
    isActive ||
    (ride.status === 'completed' && recentlyEnded);

  return {
    cardClass,
    chipDimmed: recentlyEnded,
    menuHidden: !menuVisible,
    rateEnabled: ride.status === 'completed' && recentlyEnded,
    showGlobalRating: !!endedAt && !recentlyEnded,
  };
}
...
function rideCard(ride, user, past) {
  ...
  // ============ DRIVER CHIP (passenger's view of the Nexar) ============
  const driverUser = (!isDriver && ride.driver) ? window.app.store.getUserByEmail(ride.driver) : null;
  const driverChip = (!isDriver && driverUser) ? buildDriverChip(ride, user, driverUser, state) : '';

  // ============ PASSENGER LIST (driver's view) ============
  const passengerList = isDriver ? buildPassengerList(ride, user, state) : '';

  // ============ CO-PASSENGER LIST (passenger's view of others) ============
  const coPassengerList = (!isDriver && (ride.passengers || []).length > 0) ? `
    <div class="passenger-list">
      <div class="ride-subsection-head"><span class="meta">Riders</span><span class="ride-subsection-count">${ride.passengers.length}</span></div>
      ${(ride.pickupOrder && ride.pickupOrder.length > 0 ? ride.pickupOrder : ride.passengers).map((email, i) => {
        ...
        // Passengers can only contact the Nexar, not other riders.
        // Co-passenger rows stay read-only (name + status pill only).
        const menuHtml = '';

        return `
          <div class="passenger-row" ${state.chipDimmed ? 'style="opacity:0.72;"' : ''}>
            <div class="passenger-num">${i + 1}</div>
            <div class="passenger-body">
              <div class="passenger-row-heading">
                <div class="passenger-name">${escapeHtml(name)}${isMe ? ' <span style="color:var(--text-tertiary); font-size:11px;">(you)</span>' : ''}</div>
...
// ——— Driver chip (passenger viewing the Nexar) ———
function buildDriverChip(ride, user, driverUser, state) {
  const avg = window.app.store.getAverageRating(driverUser.email);

  // After 3h from completion, no menu — just a global rating badge.
  if (state.showGlobalRating) {
    const ratingBadge = avg
      ? `<div class="driver-chip-rating-readonly"><span class="star-glyph">★</span><span>${avg.avg}</span><span style="color:var(--text-tertiary); font-weight:500;">· ${avg.count} review${avg.count === 1 ? '' : 's'}</span></div>`
      : `<div class="driver-chip-rating-readonly empty">No ratings yet</div>`;
    return `
      <div class="driver-chip ride-contact-card">
        <div class="driver-chip-avatar">${(driverUser.name || '?').trim()[0].toUpperCase()}</div>
        <div class="driver-chip-body">
  ...
  // Menu only when the ride is active or recently completed.
  const menuHtml = state.menuHidden ? '' : buildChipMenu({
    ride, user,
    targetEmail: driverUser.email, targetUser: driverUser,
    variant: 'driver',
    state,
    isDriverChipForPassenger: true,
  });
...
// ——— Passenger list (driver's view of the Nexirians) ———
function buildPassengerList(ride, user, state) {
  return `
    <div class="passenger-list">
      <div class="ride-subsection-head"><span class="meta">Passengers</span><span class="ride-subsection-count">${ride.passengers.length}</span></div>
      ${(ride.pickupOrder && ride.pickupOrder.length > 0 ? ride.pickupOrder : ride.passengers).map((email, i) => {
        ...
        // Menu visible only in active or recently-completed state.
        const menuHtml = state.menuHidden ? '' : buildChipMenu({
          ride, user,
          targetEmail: email, targetUser: p,
          variant: 'passenger', // Call + WA + Onboard + Dropped + Rate
          state,
          statusPill,
          picked, dropped,
        });
...
// variant: 'passenger' (driver viewing passenger) → Call, WA, Onboard, Dropped, Rate
// variant: 'driver'    (passenger viewing driver)   → Call, WA, Rate
function buildChipMenu({ ride, user, targetEmail, targetUser, variant, state, statusPill, picked, dropped }) {
  // --- Contact buttons (Call + WhatsApp) ---
  const contactBtns = (targetUser && targetUser.mobile) ? `
    <div class="passenger-contact-buttons">
      <a href="${buildTelUrl(targetUser.mobile)}" class="btn btn-secondary btn-sm passenger-contact-btn" title="Call">${iconPhone()}<span>Call</span></a>
      <a href="${buildWhatsAppUrl(targetUser.mobile, `Hi ${targetUser.name}, about our Nexar ride on ${ride.date}.`)}" target="_blank" rel="noopener" class="btn btn-secondary btn-sm passenger-contact-btn" title="WhatsApp">${iconWhatsApp()}<span>WhatsApp</span></a>
    </div>
  ` : '';

  // --- Action buttons: only for the driver-side variant, only while active ---
  let actionBtns = '';
  if (variant === 'passenger' && ride.status === 'active') {
    const onboardDisabled = picked ? 'disabled' : '';
    const droppedDisabled = (!picked || dropped) ? 'disabled' : '';
    ...
    actionBtns = `
      <div class="passenger-action-buttons">
        <button class="btn btn-sm passenger-action-btn ${onboardIsPrimary}"
          ...
          ${onboardDisabled}
          onclick="app.markPickedUp('${ride.id}', '${targetEmail}', this)">${picked ? 'Onboarded ✓' : 'Onboard'}</button>
        <button class="btn btn-sm passenger-action-btn ${droppedIsPrimary}"
          data-drop-btn="${ride.id}|${targetEmail}"
          ${droppedDisabled}
          onclick="app.markDropped('${ride.id}', '${targetEmail}', this)">${dropped ? 'Dropped ✓' : 'Dropped'}</button>
      </div>
    `;
  }

  // --- Rating block: only on recently-completed rides ---
  let reviewBlock = '';
  if (state.rateEnabled) {
    const existing = window.app.store.getRatingFor(ride.id, user.email, targetEmail);
    if (existing) {
      reviewBlock = `
        <div class="passenger-review-note">
          You rated ${'★'.repeat(existing.stars)}${'☆'.repeat(5 - existing.stars)}
          ${existing.comment ? `<div class="passenger-review-comment">"${escapeHtml(existing.comment)}"</div>` : ''}
        </div>
      `;
    } else {
      reviewBlock = `
        <div class="passenger-review-block" data-rate-row="${ride.id}|${targetEmail}">
          <div class="star-picker" data-star-target="${ride.id}|${targetEmail}">
            ${[1,2,3,4,5].map(n => `
              <button type="button" class="star-btn" data-stars="${n}" onclick="app.pickStars('${ride.id}', '${targetEmail}', ${n}, this)">★</button>
            `).join('')}
          </div>
          <div class="rate-comment-row" style="margin-top:6px; display:none;" data-comment-for="${ride.id}|${targetEmail}">
            <input type="text" class="field-input" style="height:36px; font-size:13px;" placeholder="Comment (optional)" data-comment-input="${ride.id}|${targetEmail}">
            <button class="btn btn-primary btn-sm" style="margin-top:6px; width:100%;" onclick="app.submitRating('${ride.id}', '${targetEmail}', this)">Submit rating</button>
          </div>
        </div>
      `;
    }
  ...
  // If nothing would show, return empty (no menu trigger rendered).
  if (!contactBtns && !actionBtns && !reviewBlock) return '';
