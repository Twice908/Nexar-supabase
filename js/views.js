...
function buildMapAndShareRow(ride, isDriver, user, state) {
  if (ride.status === 'no match' || ride.status === 'cancelled') return '';
  // After the 3h rating window closes, the card goes fully read-only.
  if (state && state.showGlobalRating) return '';
...
export function actionButtons(ride, isDriver) {
  ...
  if (isDriver) {
    ...
    if (ride.status === 'active') {
      const total = (ride.passengers || []).length;
      const dropped = (ride.droppedOff || []).length;
      const allDropped = total > 0 && dropped === total;
      if (allDropped) {
        ...
        btns.push(`<button class="btn btn-primary btn-sm" style="flex:1" disabled title="Mark all passengers dropped first">${icons.check} Complete ${dropped}/${total}</button>`);
      }
    }
  } else {
    if (ride.status === 'matched') {
